...
##   1: FOLDER		: The file path relative to /root (/) of the folder which will be checked for files. This is Recurive, any files which match the find query (folder and age) within each sub-folder will be deleted.
...
# Deletion options
FOLDER="$1"           		# The file path, this is recursive (files within sub-directories will be deleted)
EXTENSION="$2"           	# The file extension used to target files, specify 'any' to target all
...
then
	...
	# Check if a specific extension should be used
	if [ "$2" = 'any' ]; 

	# If 'any' has been passed, dont search by extension
	then
		unset EXTENSION
	...
	#Run the script
	find $FOLDER -type f -name "*$EXTENSION" -mtime +$AGE -exec rm {} +> /dev/null 2>&1
